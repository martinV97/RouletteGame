...
public class FileMannager {
	...
	public void writeJugadas(ArrayList<String> lanzamientos,String ruta) {
		try {
			fileWriter = new FileWriter(new File(ruta),true);
			for (int i = 0; i < lanzamientos.size(); i++) {
				fileWriter.write(lanzamientos.get(i)+"\n");
			}
			fileWriter.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void cleanFile(String ruta) {
		try {
			fileWriter = new FileWriter(new File(ruta),true);
			fileWriter.write("");
			fileWriter.close();
